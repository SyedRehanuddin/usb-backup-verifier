...
fetch("/api/health")
    ...
    .then((health) => {
        if (health.status !== "ok") throw new Error("Engine unavailable");
        engineStatus.classList.add("ready");
        engineStatus.lastElementChild.textContent = "CRC-32 engine ready";
    })
    .catch(() => {
        engineStatus.classList.add("unavailable");
        engineStatus.lastElementChild.textContent = "CRC-32 engine unavailable";
    });
...
function filenameFromDisposition(value) {
    const match = typeof value === "string" ? value.match(/filename="([^"]+)"/i) : null;
    return match ? match[1] : "usb_backup_verification_report.csv";
}
...
async function downloadHistoryCsv(recordId, button, spinner, label, showError) {
    ...
    try {
        const response = await fetch(`/api/history/${encodeURIComponent(recordId)}/csv`);
        if (!response.ok) {
            const error = await response.json().catch(() => ({}));
            throw new Error(
                typeof error.detail === "string"
                    ? error.detail
                    : "Unable to export the CSV report.",
            );
        }
...
async function loadCurrentReference() {
    try {
        const response = await fetch("/api/reference/current");
        if (!response.ok) throw new Error("Unable to read saved reference.");
...
async function generateReference() {
    ...
    try {
        const response = await fetch("/api/reference/generate-selected", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ selection_id: sourceSelection.selection_id }),
        });
        ...
        showFeedback(
            "success",
            `Reference created • ${result.file_count} ${result.file_count === 1 ? "file" : "files"} processed with CRC-32`,
        );
    } catch (error) {
        showFeedback(
            "error",
...
async function verifyBackup() {
    ...
    try {
        const response = await fetch("/api/verification/run-selected", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                selection_id: backupSelection.selection_id,
                record_history: true,
            }),
        });
        const result = await response.json().catch(() => ({}));
...
async function requestNativeFolder(purpose, datasetId = null) {
    const response = await fetch("/api/folders/select", {
        ...
        body: JSON.stringify({
            purpose,
            ...(appMode === "demo" ? {dataset_id: datasetId} : {}),
        }),
    });
    const selection = await response.json().catch(() => ({}));
    if (!response.ok) {
        throw new Error(
            typeof selection.detail === "string"
                ? selection.detail
...
function releaseNativeFolder(selection) {
    if (!selection) return;
    void fetch(`/api/folders/${encodeURIComponent(selection.selection_id)}`, {
        method: "DELETE",
    });
}
...
async function runLabSimulation() {
    ...
    try {
        ...
        const response = await fetch("/api/error-lab/run-selected", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                selection_id: labBackupSelection.selection_id,
                method: labSimulationMethod,
                target_path: definition.requiresTarget ? targetPath : null,
            }),
        });
        const result = await response.json().catch(() => ({}));
        if (!response.ok) {
            throw new Error(
                typeof result.detail === "string" ? result.detail : "Unable to run the simulation.",
            );
        }
        if (!isValidVerificationResponse(result)) {
            throw new Error("The verification service returned an invalid response.");
...
async function loadHistory() {
    ...
    try {
        ...
        const result = await response.json().catch(() => ({}));
        if (!response.ok) {
            throw new Error(typeof result.detail === "string" ? result.detail : "Unable to load verification history.");
        }
        if (!result || !Array.isArray(result.records) || !result.records.every(isHistorySummaryRecord)) {
...
async function loadHistoryDetail(recordId, trigger) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(typeof record.detail === "string" ? record.detail : "Unable to load verification details.");
        }
...
async function confirmHistoryClear() {
    ...
    try {
        ...
        const result = await response.json().catch(() => ({}));
        if (!response.ok) {
            throw new Error(typeof result.detail === "string" ? result.detail : "Unable to clear verification history.");
        }
        closeHistoryDetail();
        closeHistoryClearDialog();
        await loadHistory();
        setHistoryFeedback("success", `${result.deleted} ${result.deleted === 1 ? "record" : "records"} cleared. Trusted reference preserved.`);
...
if (appMode === "demo") {
    void (async () => {
        await selectVerificationFolder("source");
